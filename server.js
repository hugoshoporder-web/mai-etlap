...
async function loadData() {
  ...
  rows.forEach(r => {
    ...
    map[key] ??= {};
    map[key][r.datum] ??= {};
    map[key][r.datum][r.tipus] ??= [];
    map[key][r.datum][r.tipus].push(r.etel);
  });
...
<meta charset="utf-8">${style}</head><body>`);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (data[iso(d)] && d > tomorrow) futureDays.push(d);
  }
  ...
  res.write(`<hr><img src="${qr}" alt="QR"><p>${pageUrl}</p><p>~10s Server Wake-Up</p>
  <p style="font-weight:bold;text-align:center">by István Gris</p>`);

  res.write(`</body></html>`);
  res.end();
});

app.listen(port, () => console.log("Menü szerver fut – visszalink + heti szekciók rendben"));
