...
app.get("/", async (req,res)=>{
  ...
  etteremek.forEach(e=>{
    res.write(`<li>/etterem/${e}▶ ${e}</a></li>`);
  });
  res.write(`</ul><hr>`);

  const qr=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(baseUrl)}`;
  res.write(`${qr}<p style="font-weight:bold;text-align:center">by István Gris</p>`);
  res.write(`</body></html>`);
  res.end();
});
...
app.get("/etterem/:etterem", async (req,res)=>{
  ...
  const baseUrl=`${req.protocol}://${req.get("host")}`;
  const pageUrl=`${baseUrl}/etterem/${etterem}`;

  res.write(`<!doctype html><html><head><meta charset=utf-8>${style}</head><body>`);
  res.write(`<a href="/">&larr; Vissza az éttermekhez</a>`);
  ...
  if(future.length){
    res.write(`<hr class="divider"><details open><summary>Aktuális hét további napjai</summary>`);
    future.forEach(d=>{
  ...
  res.write(`<hr>${qr}<p>${pageUrl}</p><p style="font-weight:bold;text-align:center">by István Gris</p>`);

  res.write(`</body></html>`);
  res.end();
});

app.listen(port,()=>console.log("Menü szerver fut – főoldal + étteremoldal kész"));
